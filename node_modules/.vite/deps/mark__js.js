...
//#region node_modules/mark.js/dist/mark.js
			...
			createClass(DOMIterator, [
				...
				},
				{
					...
					value: function isIframeBlank(ifr) {
						var bl = "about:blank", src = ifr.getAttribute("src").trim();
				...
				},
				{
					...
					value: function forEachIframe(ctx, filter, each) {
						...
						ifr.forEach(function(ifr) {
							if (DOMIterator.matches(ifr, _this3.exclude)) checkEnd();
							else _this3.onIframeReady(ifr, function(con) {
								if (filter(ifr)) {
									handled++;
									each(con);
								}
				...
				},
				{
					...
					value: function handleOpenIframes(ifr, whatToShow, eCb, fCb) {
						...
						ifr.forEach(function(ifrDict) {
							if (!ifrDict.handled) _this4.getIframeContents(ifrDict.val, function(con) {
				...
				},
				{
					...
					value: function iterateThroughNodes(whatToShow, ctx, eachCb, filterCb, doneCb) {
						...
						var ifr = [], elements = [], node = void 0, prevNode = void 0, retrieveNodes = function retrieveNodes() {
						...
						while (retrieveNodes()) {
							if (this.iframes) this.forEachIframe(ctx, function(currIfr) {
				...
				},
				{
					...
					value: function forEachNode(whatToShow, each, filter) {
						...
						contexts.forEach(function(ctx) {
							...
							if (_this6.iframes) _this6.waitForIframes(ctx, ready);
							else ready();
						});
					}
				}
			...
			createClass(Mark, [
				{
					...
					value: function log(msg) {
						...
						if ((typeof log === "undefined" ? "undefined" : _typeof(log)) === "object" && typeof log[level] === "function") log[level]("mark.js: " + msg);
					}
				},
				{
					key: "escapeStr",
				...
				},
				{
					...
					value: function createSynonymsRegExp(str) {
						...
						for (var index in syn) if (syn.hasOwnProperty(index)) {
				...
				},
				{
					...
					value: function createDiacriticsRegExp(str) {
						var sens = this.opt.caseSensitive ? "" : "i", dct = this.opt.caseSensitive ? [
				...
				},
				{
					...
					value: function getSeparatedKeywords(sv) {
						...
						sv.forEach(function(kw) {
							...
							} else kw.split(" ").forEach(function(kwSplitted) {
				...
				},
				{
					...
					value: function checkRanges(array) {
						...
						array.sort(function(a, b) {
							...
							var _callNoMatchOnInvalid = _this3.callNoMatchOnInvalidRanges(item, last), start = _callNoMatchOnInvalid.start, end = _callNoMatchOnInvalid.end;
							if (_callNoMatchOnInvalid.valid) {
								item.start = start;
				...
				},
				{
					...
					value: function getTextNodes(cb) {
						...
						this.iterator.forEachNode(NodeFilter.SHOW_TEXT, function(node) {
							...
							if (_this4.matchesExclude(node.parentNode)) return NodeFilter.FILTER_REJECT;
						...
						this.getTextNodes(function(dict) {
							dict.nodes.forEach(function(node) {
								...
								while ((match = regex.exec(node.textContent)) !== null && match[matchIdx] !== "") {
									...
									var pos = match.index;
									if (matchIdx !== 0) for (var i = 1; i < matchIdx; i++) pos += match[i].length;
					...
					value: function wrapMatchesAcrossElements(regex, ignoreGroups, filterCb, eachCb, endCb) {
						...
						this.getTextNodes(function(dict) {
							...
							while ((match = regex.exec(dict.value)) !== null && match[matchIdx] !== "") {
								...
								if (matchIdx !== 0) for (var i = 1; i < matchIdx; i++) start += match[i].length;
					...
					value: function wrapRangeFromIndex(ranges, filterCb, eachCb, endCb) {
						...
						this.getTextNodes(function(dict) {
							...
							ranges.forEach(function(range, counter) {
								var _checkWhitespaceRange = _this8.checkWhitespaceRanges(range, originalLength, dict.value), start = _checkWhitespaceRange.start, end = _checkWhitespaceRange.end;
								if (_checkWhitespaceRange.valid) _this8.wrapRangeInMappedTextNode(dict, start, end, function(node) {
				...
				},
				{
					...
					value: function markRegExp(regexp, opt) {
						...
						this[fn](regexp, this.opt.ignoreGroups, function(match, node) {
							...
							if (totalMatches === 0) _this9.opt.noMatch(regexp);
				...
				},
				{
					...
					value: function mark(sv, opt) {
						...
						this.opt = opt;
						var totalMatches = 0, fn = "wrapMatches";
						var _getSeparatedKeywords = this.getSeparatedKeywords(typeof sv === "string" ? [sv] : sv), kwArr = _getSeparatedKeywords.keywords, kwArrLen = _getSeparatedKeywords.length, sens = this.opt.caseSensitive ? "" : "i", handler = function handler(kw) {
							...
							_this10[fn](regex, 1, function(term, node) {
								...
								if (matches === 0) _this10.opt.noMatch(kw);
								if (kwArr[kwArrLen - 1] === kw) _this10.opt.done(totalMatches);
